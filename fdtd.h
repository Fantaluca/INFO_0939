...
#define INDEX3D(grid, m, n, p)                                                 \
  ((size_t)grid.numnodesy * grid.numnodesx * (m) + grid.numnodesx * (n) + (p))
...
typedef struct simulation_data_rank {
  parameters_t params;

  grid_rank_t grid;

  data_rank_t *c, *rho, *rhohalf;

  data_rank_t *pold, *pnew;
  data_rank_t *vxold, *vxnew;
  data_rank_t *vyold, *vynew;
  data_rank_t *vzold, *vznew;

} simulation_data_rank_t;
 ...
 */
void closest_index(grid_rank_t *grid_rank, double x, double y, double z, int *cx, int *cy,
                   int *cz);
 ...
 *        the grid provided in argument
 *
 * @param grid_rank [IN] the grid describing the 3D array to allocate
 * @return data_t* return a new data object or NULL if allocation failed
 */
data_rank_t *allocate_data_rank(grid_rank_t *grid);
 ...
 * @param value [IN] the value used to fill the array
 */
void fill_data_rank(simulation_data_rank_t *simdata_rank,data_rank_t *data, double value);
 ...
 */
void apply_source(simulation_data_rank_t *simdata, int step);
 ...
 */
void update_pressure(simulation_data_rank_t *simdata);
 ...
 * store result of the update step
 */
void update_velocities(simulation_data_rank_t *simdata);
